...
flagQRCode = False
flagDifferentialReceived = False
...
class NodeDecisionMaker():
    def __init__(self):
        ...
        self.subDifferential = rospy.Subscriber(
            'TPC4Differential', Float64, self.callbackDifferential)
    ...
    def callbackDifferential(self, msg_Differential):
        global flagDifferentialReceived
        flagDifferentialReceived = True
        self.msgDifferential = int(msg_Differential.data)
    ...
    while not rospy.is_shutdown():
        try:
            if dm.timeMin < time.time() - dm.tSendMsgCAN:
                ...
                differential = nodeDecisionMaker.msgDifferential
                ...
                if (flagBreakAEB or flagBreakYOLO):
                    ...
                    canParams = [1, (dm.rpm_can + differential), 1, (dm.rpm_can - differential)]
                    print("angDir: ", angDir)
                    print("rpmDir: ", dm.rpm_can - differential)
                    print("rpmEsq: ", dm.rpm_can + differential)
                    vc.sendMsg(s, canID, canParams)
                    if flagThrottle:

...
def binary_thresholder(img, limiar):
    ...
    (T, thresh) = cv2.threshold(blurred, limiar, 255, cv2.THRESH_BINARY_INV) #indoor
...
def main():
    ...
    while True:
        try:  
            ...
            # imagem binarizadda
            img_bin = binary_thresholder(img_be, limiar)
            cv2.imshow('bin', img_bin)
            #cv2.waitKey(0)
            
            # exibindo pontos da linha esquerda e direita
            leftx, lefty, rightx, righty, left_lane_inds, right_lane_inds = find_lane_pixels_using_histogram(img_bin)
            #print(len(left_lane_inds), len(right_lane_inds))

            # Verificando quantidade de pixels nao nulos e alterando thresholder para atender a condicao
            if len(left_lane_inds) < 2500 or len(right_lane_inds) < 2500:
                limiar += 1
                print("limiar:", limiar)
                
            else: #len(left_lane_inds) > 2500 or len(right_lane_inds) > 2500:
                limiar -= 1
                print("limiar:", limiar)

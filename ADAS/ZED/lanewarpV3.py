...
class LaneWarp():
    ...
    def pure_pursuit(self, distance_to_center):

        offSet = 1.05  # Distância de alcance do ponto de perseguição

        wheelbase = 0.75  # Distância entre eixos 

        target_point = [offSet, distance_to_center]

        # Calcula o ângulo de direção necessário para atingir o ponto de perseguição
        alpha = math.atan2(target_point[1], target_point[0])

        # Calcula o ângulo das rodas do veículo com base no ângulo de direção
        wheel_angle = math.atan(2 * wheelbase * math.sin(alpha) / offSet)

        # Mapeia o ângulo das rodas do intervalo [-pi/2, pi/2] para o intervalo [1, 50]
    ...
    def working(self, imagem, NodeRos): 
        ...
        angDir = int(self.pure_pursuit(veh_pos))

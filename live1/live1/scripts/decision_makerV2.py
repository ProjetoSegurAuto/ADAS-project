...
class DecisionMaker():
    ...
    def AEB(self, distance, angDir):
        ...
        try:
            ...
            self.sendInfoSotftware(angDir, 0, 0)
    ...
    while not rospy.is_shutdown():
        ...
        try:
            if dm.timeMin < time.time() - dm.tSendMsgCAN:
                ...
                breaking = dm.AEB(nodeDecisionMaker.msgDepth, angDir)

                if not breaking:
                    msgCanId = 0x56
                    param = [1, rpmEsq, 1, rpmDir]
                    #vc.sendMsg(s, msgCanId, param)
                    # sempre enviar msg para o software
                    #dm.sendInfoSotftware(angCan=angDir, rpmDir=rpmDir, rpmEsq=rpmEsq)
                    # vc.logCAN(s)
                else:
                    pass
                ...
                if flagObjectYOLOReceived == True:
                    ...
                    for object in jsonObjectYOLO:
                        if (jsonObjectYOLO[object]['classId'] == "stop sign"):
                            ...
                            dm.sendInfoSotftware(angCan=angDir, rpmDir=0, rpmEsq=0)
            ...
            param = [angDir]
            vc.sendMsg(s, msgCanId, param)
            dm.sendInfoSotftware(angCan=angDir, rpmDir=rpm, rpmEsq=rpm)
            s.close()
            break

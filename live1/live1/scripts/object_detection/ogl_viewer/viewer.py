...
class GLViewer:
            ...
            if self.render_object(_objs.object_list[i]):
                ...
                if bounding_box.any():
                    ...
                    if _objs.object_list[i].tracking_state != sl.OBJECT_TRACKING_STATE.OK:
                        ...
                        color_white = get_color_class(5)
                        pos = [_objs.object_list[i].position[0], _objs.object_list[i].bounding_box[0][1], _objs.object_list[i].position[2]]
                        pos_L = [_objs.object_list[i].position[0], _objs.object_list[i].bounding_box[0][1]-0.1, _objs.object_list[i].position[2]]
                        pos_D = [_objs.object_list[i].position[0], _objs.object_list[i].bounding_box[0][1]-0.2, _objs.object_list[i].position[2]]
                        pos_v = [_objs.object_list[i].position[0], _objs.object_list[i].bounding_box[0][1]-0.3, _objs.object_list[i].position[2]]
                        self.create_id_rendering(pos, color_white, _objs.object_list[i].id)
                        self.create_labels_rendering(pos_L, color_white, _objs.object_list[i])   
                        self.create_velocity_rendering(pos_v, color_white, np.around(np.linalg.norm(_objs.object_list[i].velocity), 2))
                        self.create_distance_rendering(pos_D, color_white, np.around(np.linalg.norm(_objs.object_list[i].position), 2))             
    ...
    def create_bbox_rendering(self, _bbox, _bbox_clr):
        self.BBox_edges.add_full_edges(_bbox, _bbox_clr) # First create top and bottom full edges  
        self.BBox_edges.add_vertical_edges(_bbox, _bbox_clr) # Add faded vertical edges
        self.BBox_faces.add_vertical_faces(_bbox, _bbox_clr) # Add faces
        self.BBox_faces.add_top_face(_bbox, _bbox_clr) # Add top face
    ...
    def create_velocity_rendering(self, _center, _clr, _vel):
        tmp = ObjectClassName()
        tmp.name = str(_vel) + "m/s"
        tmp.color = _clr
        tmp.position = np.array([_center[0], _center[1], _center[2]], np.float32)
        self.objects_name = np.append(self.objects_name, tmp)

    def create_distance_rendering(self, _center, _clr, _dist):
        tmp = ObjectClassName()
        tmp.name = str(_dist) + "m"
        tmp.color = _clr
        tmp.position = np.array([_center[0], _center[1], _center[2]], np.float32)
        self.objects_name = np.append(self.objects_name, tmp)

    def create_labels_rendering(self, _center, _clr, _objs):
        tmp = ObjectClassName()
        tmp.name = str(_objs.label)
        if(hasattr(_objs, 'sublabel') and str(_objs.sublabel)!=str(_objs.label)):
            tmp.name = tmp.name+': '+str(_objs.sublabel)

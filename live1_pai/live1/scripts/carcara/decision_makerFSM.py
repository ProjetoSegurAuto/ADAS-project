...
GAP_MIN = 1.3
GAP_INCREMENT = 0.2
CHANGE_GAP_AFTER = 500

DISTANCE_ACC = 1.6 
DISTANCE_STOP = 1.2 #0.8
...
class DecisionMakerFSM:
    ...
    def adjust_gap(self):
        global gap_global_counter, CHANGE_GAP_AFTER

        if gap_global_counter >= CHANGE_GAP_AFTER:
            self.gap += GAP_INCREMENT
            gap_global_counter = 0
        else:
            gap_global_counter += 1
    ...
    def actions(self, node_decision_maker):
        ...
        if self.state == 0:
            ...
            if self.time_min < time.time() - self.t_send_msg_can:
                ...
                msg_can_id = 0x94
                param = [3, int(self.gap*100), 69, 70, ang_dir, self.rpm_can, self.rpm_can, msg_can_id]
                node_decision_maker.pubOrinToInfra(param)

                print("GAP: {}".format(int(self.gap*100)))
